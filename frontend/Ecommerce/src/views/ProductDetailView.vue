  ...
  <div class="bg-gradient-to-b from-neutral-950 via-neutral-900 to-black text-white min-h-[calc(100vh-5rem)] pt-24 px-4">
    <div class="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
      ...
      <div class="flex flex-col items-center">
        ...
        <div v-else class="w-full max-w-md bg-neutral-900/60 border border-neutral-800 rounded-2xl shadow-xl ring-1 ring-black/5 backdrop-blur-sm p-4">
          <div class="relative w-full aspect-square overflow-hidden rounded-xl">
            <img :src="currentImage || placeholder" :alt="product?.name" class="w-full h-full object-contain transition-transform duration-300 hover:scale-[1.02]" @error="onMainImgError" />
          </div>
          ...
          <div v-if="thumbs.length" class="mt-4 grid grid-cols-3 gap-3">
            <button v-for="thumb in thumbs" :key="thumb.index" type="button"
              ...
              <img :src="thumb.src || placeholder" :alt="product?.name" class="w-full aspect-square object-contain transition-all duration-200 hover:opacity-90" @error="onThumbImgError" />
            </button>
          </div>
        </div>
      </div>
        ...
        <template v-else>
          ...
          <div class="mb-4 flex flex-wrap items-center gap-3">
            <span v-if="product?.code || brandName"
              ...
              <svg viewBox="0 0 24 24" class="w-4 h-4" aria-hidden="true"><path fill="currentColor" d="M3 7h18v2H3V7zm0 4h12v2H3v-2zm0 4h18v2H3v-2z"/></svg>
            ...
            <div class="flex items-center gap-2" :aria-label="`Avaliação ${averageStarsFixed} de 5`" role="img">
              <div class="flex">
                <svg v-for="(s, i) in starIcons" :key="i" viewBox="0 0 24 24" class="w-4 h-4 mx-[1px]">
                  <defs><linearGradient :id="`grad-${i}`" x1="0" y1="0" x2="1" y2="0">
                    <stop offset="0" stop-color="currentColor"/><stop :offset="s === 'half' ? '50%' : '0%'" stop-color="currentColor"/>
                    <stop :offset="s === 'half' ? '50%' : '0%'" stop-color="transparent"/><stop offset="100%" stop-color="transparent"/>
                  </linearGradient></defs>
          ...
          <div class="rounded-2xl bg-neutral-900/60 border border-neutral-800 ring-1 ring-black/5 shadow-lg divide-y divide-neutral-800 overflow-hidden">
            <div class="p-5">
              ...
              <p class="text-neutral-300 leading-relaxed whitespace-pre-line">{{ descriptionDisplay }}</p>
              <button v-if="isDescLong" @click="showDesc = !showDesc" class="mt-3 text-sm font-medium text-orange-400 hover:text-orange-300 transition-colors" :aria-expanded="showDesc">
                {{ showDesc ? 'Ver menos' : 'Ler mais' }}
              </button>
            </div>

            <div v-if="techText" class="p-5">
              ...
              <button v-if="isTechLong" @click="showTech = !showTech" class="mt-3 text-sm font-medium text-orange-400 hover:text-orange-300 transition-colors" :aria-expanded="showTech">
...
const techText = computed(() => product.value?.technical_info ?? product.value?.technicalInfo ?? '')
...
watch(() => [product.value?.brand_id, product.value?.brandId, productBrandName.value], async () => { await loadBrandIfNeeded() })
