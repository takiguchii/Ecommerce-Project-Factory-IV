        ...
        <div class="lg:col-span-2 space-y-6">
          <div class="space-y-4">
            <div v-for="item in cart.items" :key="item.productId"
              ...
              <div class="flex-1 min-w-0">
                ...
                <button @click="removeFromCart(item.productId)" class="mt-3 text-sm text-red-400 hover:text-red-300">
                  Remover
                </button>
              </div>
            </div>
          ...
          <div class="bg-neutral-900 border border-neutral-800 rounded-xl p-6">
            ...
            <div v-else-if="hasAddress">
              ...
              <div v-if="shippingOptions.length > 0" class="space-y-2">
                <div v-for="option in shippingOptions" :key="option.name" @click="selectedShipping = option"
                  class="flex items-center justify-between p-3 rounded-lg border cursor-pointer transition-all hover:bg-neutral-800"
                  :class="selectedShipping?.name === option.name
                      ? 'border-orange-500 bg-neutral-800'
                      : 'border-neutral-700'
                    ">
                  <div class="flex items-center gap-3">
                    <div class="w-4 h-4 rounded-full border flex items-center justify-center" :class="selectedShipping?.name === option.name
                        ? 'border-orange-500'
                        : 'border-neutral-500'
                      ">
                      <div v-if="selectedShipping?.name === option.name" class="w-2 h-2 rounded-full bg-orange-500">
                      </div>
                    </div>
                    <div>
          ...
          <div class="bg-neutral-900 border border-neutral-800 rounded-xl p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <button @click="selectedPayment = 'PIX'"
                class="flex items-center gap-3 p-4 rounded-lg border-2 transition-all hover:bg-neutral-800" :class="selectedPayment === 'PIX'
                    ? 'border-orange-500 bg-neutral-800'
                    : 'border-neutral-700'
                  ">
                <span class="text-2xl">💠</span>
              ...
              <button @click="selectedPayment = 'CREDITCARD'"
                class="flex items-center gap-3 p-4 rounded-lg border-2 transition-all hover:bg-neutral-800" :class="selectedPayment === 'CREDITCARD'
                    ? 'border-orange-500 bg-neutral-800'
                    : 'border-neutral-700'
                  ">
                <span class="text-2xl">💳</span>
        ...
        <aside class="bg-neutral-900 border border-neutral-800 rounded-xl p-5 h-fit sticky top-6">
          ...
          <div class="flex justify-between text-neutral-300 mb-4 mt-4 border-t border-neutral-800 pt-4">
            ...
            <span class="font-bold text-orange-400 text-xl">
              R$
              {{
                toFixed2(cartTotal + (selectedShipping?.price || 0))
              }}
            </span>
          </div>

          <button @click="handleCheckout" :disabled="isCheckoutLoading || !selectedPayment || !selectedShipping"
...
const handleCheckout = async () => {
  ...
  try {
    ...
    const total = totalItemsValue + shippingPrice
    const customerName =
      profile.value?.fullName || profile.value?.username || 'Cliente'
    const customerDocument = profile.value?.documentCpfCnpj || ''
    const customerEmail = profile.value?.email || ''
