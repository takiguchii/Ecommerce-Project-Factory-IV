<template>
  <section class="min-h-[60vh] bg-neutral-950 text-neutral-100">
    <div class="max-w-6xl mx-auto p-4 md:p-8">
      <h1 class="text-2xl md:text-3xl font-bold text-orange-400 mb-6">
        Meu Carrinho
      </h1>
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="space-y-4">
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                <h3 class="font-semibold text-orange-300 truncate">
                  {{ item.name }}
                </h3>
                <div class="mt-2 flex flex-wrap items-center gap-4">
                  <p class="text-lg font-bold text-orange-400">
                    R$ {{ toFixed2(item.price) }}
                  </p>
          ...
          <div class="bg-neutral-900 border border-neutral-800 rounded-xl p-6">
              ...
              <div class="flex items-start gap-3 mb-6 pb-4 border-b border-neutral-800">
                ...
                <div>
                  <p class="text-neutral-300 font-medium">
                    {{ profile.addressLine || 'Endereço Principal' }}
                  </p>
                  <p class="text-sm text-neutral-500">
                    {{ profile.city }} - {{ profile.state }} (CEP:
                    {{ profile.postalCode }})
                  </p>
                </div>
              </div>

              <p v-if="shippingError" class="text-red-400 text-sm mb-3">
                {{ shippingError }}
              </p>
              ...
              <div v-if="shippingOptions.length > 0" class="space-y-2">
                ...
                >
                  <div class="flex items-center gap-3">
                    ...
                    >
                      <div
                        v-if="selectedShipping?.name === option.name"
                    ...
                    <div>
                      ...
                      <p class="text-xs text-neutral-400">
                        Entrega em
                        {{ option.estimatedDeliveryDays }} dias úteis
                      </p>
                    </div>
                  </div>
                  <p class="font-bold text-orange-400">
                    R$ {{ toFixed2(option.price) }}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div v-else class="text-center py-6">
              <p class="text-neutral-300 mb-4">
                Não encontramos um endereço de entrega no seu perfil.
              </p>
              <router-link
                to="/profile"
          ...
          <div class="bg-neutral-900 border border-neutral-800 rounded-xl p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              >
                <span class="text-2xl">💳</span>
                <div class="text-left">
                  <p class="font-bold text-neutral-100">
                    Cartão de Crédito
                  </p>
                  <p class="text-xs text-neutral-400">
                    Até 12x sem juros
          ...
          <div class="bg-neutral-900 border border-neutral-800 rounded-xl p-6">
            ...
            <div class="flex gap-3">
              <input
                ...
                v-model="couponInput"
                placeholder="Código"
                class="flex-1 rounded-lg bg-neutral-800 border border-neutral-700 px-4 py-2 text-neutral-100 uppercase focus:outline-none focus:border-orange-500"
                :disabled="!!appliedCoupon"
              />
              <button
                v-if="!appliedCoupon"
                @click="handleApplyCoupon"
                class="px-6 py-2 rounded-lg border border-orange-500 text-orange-500 font-semibold hover:bg-orange-500 hover:text-black transition-all"
              >
                Aplicar
              </button>
              <button
                v-else
                @click="removeCoupon"
                class="px-6 py-2 rounded-lg border border-red-500 text-red-500 font-semibold hover:bg-red-500 hover:text-white transition-all"
              >
                Remover
              </button>
            </div>
            <p
              v-if="couponMessage"
              :class="couponError ? 'text-red-400' : 'text-green-400'"
              class="text-sm mt-2"
            >
              {{ couponMessage }}
            </p>
        ...
        <!-- Resumo -->
        <aside
          class="bg-neutral-900 border border-neutral-800 rounded-xl p-5 h-fit sticky top-6"
        >
          <h2 class="text-lg font-semibold text-neutral-200 mb-4">
            Resumo do Pedido
          </h2>
          ...
          <div
            v-if="appliedCoupon"
            class="flex justify-between text-green-400 mb-2"
          >
          ...
          <div class="flex justify-between text-neutral-300 mb-2">
            ...
            <span :class="selectedShipping ? 'text-neutral-200' : 'text-neutral-500'">
              {{
                selectedShipping
                  ? `R$ ${toFixed2(selectedShipping.price)}`
                  : '--'
              }}
            </span>
          </div>
          ...
          <div
            class="flex justify-between text-neutral-300 mb-4 mt-4 border-t border-neutral-800 pt-4"
          >
          ...
          <div class="mt-3 text-center text-xs text-neutral-500 space-y-1">
            <p v-if="!selectedShipping" class="text-orange-400/70">
              * Selecione o frete
            </p>
            <p v-if="!selectedPayment" class="text-orange-400/70">
          ...
          <p
            v-if="checkoutError"
            class="mt-3 text-sm text-red-400 text-center"
          >
            {{ checkoutError }}
          </p>
        </aside>
...
import { calculateShipping, validateCoupon } from '@/services/api'
...
const finalTotal = computed(() =>
  Math.max(
    0,
    cartTotal.value +
      (toNum(selectedShipping.value?.price) || 0) -
...
const handleCheckout = async () => {
  ...
  try {
    const itemsSnapshot = (cart.value?.items || []).map((item) => ({
      id: item.productId,
      name: item.name,
    ...
    const shippingPrice = toNum(selectedShipping.value.price)
    const totalItemsValue = cartTotal.value
    ...
    const orderSummary = {
      ...
      total,
      paymentMethod:
        selectedPayment.value === 'PIX' ? 'Pix' : 'Cartão de Crédito',
      shippingOption: {
        name: selectedShipping.value.name,
        carrier: selectedShipping.value.carrier,
        estimatedDeliveryDays: selectedShipping.value.estimatedDeliveryDays,
      },
      customerName:
        profile.value?.fullName || profile.value?.username || 'Cliente',
      userEmail: profile.value?.email || '',
      userCpfCnpj: profile.value?.documentCpfCnpj || '',
      items: itemsSnapshot,
      ...
      coupon: appliedCoupon.value
        ? {
            ...
            isPercentage: !!appliedCoupon.value.isPercentage,
            discountValue: appliedCoupon.value.discountValue ?? 0,
          }
        : null,
    }
    ...
    existingHistory.push(orderSummary)
