...
// Garante que só 12 marcas aparecem, mesmo se o backend mandar 50
const displayedBrands = computed(
  () => props.brands?.slice(0, MAX_DISPLAY_SLOTS) ?? []
)
...
const placeholderSvg =
  ...
  encodeURIComponent(
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 225"><rect width="400" height="225" fill="#18181b"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="#3f3f46" font-size="18" font-family="sans-serif">Espaço Reservado</text></svg>'
  )
</script>
...
<template>
  <section class="mt-8 px-4">
    <h2
      class="text-xl md:text-2xl font-bold text-orange-500 uppercase tracking-wide border-b-2 border-orange-500/20 pb-1 mb-8"
    >
      {{ title }}
    </h2>
    ...
    <div
      v-else-if="error"
      class="text-center text-red-400 bg-red-900/20 p-2 rounded"
    >
    ...
    <div
      v-else
      class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-8 -mx-4 sm:-mx-6"
    >
      <!-- Marcas -->
      <RouterLink
      ...
      >
        <div
          class="aspect-[4/3] w-full overflow-hidden bg-white/5 flex items-center justify-center p-4"
        >
          <img
        ...
        <div class="px-3 py-2 text-center">
          <p
            class="text-xs font-semibold text-neutral-300 group-hover:text-white transition-colors"
          >
            {{ (brand.name || '').toUpperCase() }}
          </p>
          <span
            class="mt-1 inline-flex items-center gap-1 text-[10px] font-bold text-orange-400 group-hover:text-orange-300 transition-colors"
          >
            VER OFERTAS
          </span>
        </div>
      ...
      >
        <svg
          ...
          class="w-1/4 h-1/4 text-neutral-700"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          stroke-width="2"
