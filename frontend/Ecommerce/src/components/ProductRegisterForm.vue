<template>
  <div class="min-h-screen bg-gradient-to-br from-black via-zinc-900 to-zinc-800 flex items-center justify-center p-4">
    <section class="w-full max-w-md">
      <div class="bg-white/95 backdrop-blur shadow-xl rounded-2xl border border-black/5 overflow-hidden">
        ...
        <transition name="fade">
          <div v-if="banner.type === 'success'" class="mx-6 mt-4 rounded-lg border border-emerald-300 bg-emerald-50 text-emerald-800 px-4 py-3 text-sm">
            ✅ {{ banner.message }}
          </div>
        </transition>
        <transition name="fade">
          <div v-if="banner.type === 'error'" class="mx-6 mt-4 rounded-lg border border-rose-300 bg-rose-50 text-rose-800 px-4 py-3 text-sm">
            ⚠️ {{ banner.message }}
          </div>
        </transition>
          ...
          <form @submit.prevent="onSubmit" class="space-y-4">
            ...
            <div>
              ...
              <div class="relative">
                ...
                <input v-model="form.name" type="text" placeholder="Ex.: Mouse Gamer RGB" :class="inputClass(form.name)" required/>
              </div>
            ...
            <div>
              ...
              <div class="relative">
                ...
                <textarea v-model="form.description" placeholder="Ex.: Sensor 16.000 DPI, 7 botões programáveis…" :class="textareaClass(form.description)" required></textarea>
              </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div class="relative">
                  ...
                  <input v-model="form.price" type="number" step="0.01" placeholder="199.90" :class="inputClass(form.price)" required/>
                </div>
              ...
              <div>
                ...
                <div class="relative">
                  ...
                  <input v-model="form.stock" type="number" min="0" placeholder="Ex.: 25" :class="inputClass(form.stock)" required/>
                </div>
...
async function onSubmit(){
  ...
  try{ submitting.value=true; await apiPost('/products', form); banner.type='success'; banner.message='Produto registrado com sucesso!'; form.name=''; form.description=''; form.price=''; form.stock=''; triedSubmit.value=false }
  catch(e){ banner.type='error'; banner.message=`Erro ao registrar produto: ${e.message}` }
